...
uint8_t fuel_cell_under_voltage = 0;
uint8_t fuel_cell_over_temperature = 0;
uint8_t fuel_cell_over_current = 0;

uint8_t super_capacitors_over_current = 0;
uint8_t super_capacitors_over_voltage = 0;
...
static void update_error_from_measurements(void);
...
static void calculateValues(void) {
	...
	VALUES.FC_Temp_to_average =
			(((4095.0f - (float) adcDataToCalculate[0]) * 20)) / 300.0f; //zależność wyznaczona eksperymentalnie w sali
...
static void update_error_from_measurements(void)
{
	if (VALUES.SC_C.value >= 15) {
		super_capacitors_over_current = 1;
	} else {
		super_capacitors_over_current = 0;
	}
	if (VALUES.SC_V.value >= 49) {
		super_capacitors_over_voltage = 1;
	} else {
	...
	if (VALUES.FC_TEMP.value >= 65) {
		...
		fuel_cell_over_temperature = 0;
	}
	if(VALUES.FC_CURRENT.value >= 12)
	{
		fuel_cell_over_current = 1;
	} else {
		fuel_cell_over_current = 0;
	}
}
...
void adc_step() {
	...
	calculateValues();
	averaging_Values();
	update_error_from_measurements(); //Specify values inside
}
