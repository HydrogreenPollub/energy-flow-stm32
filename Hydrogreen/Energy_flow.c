...
static void energy_flow_state_emergency_hard(void);
...
void energyflow_step(void) {
	...
	if (!rs485_flt && !emergency) {
		...
		case 0:
			...
			break;
		case 1:
			energy_flow_state_emergency_hard();
			break;
		default:
...
static void energy_flow_state_emergency_hard() {
	hydros.charging = 0;
	SC_Set_charging(hydros.charging);
	SC_State(0);
	debug_state = 0;
...
static void FC_to_SC_Current_regulator(uint8_t current) {
	...
	if (RS485_RX_VERIFIED_DATA.motorPWM == 0) {
		if (time >= SC_C_regulator.PIDtimeFactor) {
			...
			SC_C_regulator.integrator = SC_C_regulator.iError
					* SC_C_regulator.Ki;
			...
			} else if (SC_C_regulator.controlValue
					<= SC_C_regulator.controlMin) {
